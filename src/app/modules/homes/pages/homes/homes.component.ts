...
import { GetHomesList, GetSearchOptions, LoadMoreHomeList, StartLoading } from '../../store/actions/homes.actions';
...
export class HomesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.store.dispatch(new GetHomesList({_page: String(this._pageIndex), _limit: String(this._itemsPerPage)}));
  ...
  private listenToLoadMore(){
    this.loadMore$.subscribe(() => {
      ...
      this.isLoading = true;
      this._pageIndex += 1;
      this.store.dispatch(new LoadMoreHomeList({_page:String(this._pageIndex), _limit: String(this._itemsPerPage)}));
    })
  }
  ...
  private listenScrollAndResize(){
      ...
      filter(() => {
        return  (window.innerHeight + window.scrollY )>= document.body.clientHeight && !this.isLoading && this.canLoadMore
      })
    ).subscribe(() => {
      this.loadMore$.next();
    });
  ...
  private listenToLoadingAbility(){
    this.store.pipe(
      ...
      select(fromHomes.homes.canLoadMore),
    ).subscribe(canLoad => {
  ...
  private listenToLoadingStatus(){
    this.store.pipe(
      takeUntil(this.destruct$),
      select(fromHomes.homes.loading)
      ...
      map(([items, filter]) => {
        ...
        if((searchKey.length > 2 &&
          searchKey.length >= this.previousSearchKey.length) ||
                (searchKey.length < this.previousSearchKey.length && searchKey.length <= 3)){
           return items.filter((item) => {         
             const normalizedItemName = this._normalizeValue(item.name);
             const normalizedItemAddress = this._normalizeValue(item.address);
             const normalizedFilterSearchKey = this._normalizeValue(filter.searchKey);
             return normalizedItemName.indexOf(normalizedFilterSearchKey) > -1 || normalizedItemAddress.indexOf(normalizedFilterSearchKey) > -1
           }
          )
        }
         else return items;
      ...
      map(([filteredOptions, filter]) => {
        return filteredOptions
          ? filteredOptions.filter((item) =>{
            return this._normalizeValue(item.match).includes(filter.searchKey)
          }
        )
        : [];
      })
  ...
  private _normalizeValue(value: string): string {
    const normalizedVal = value.toLowerCase().replace(/\s/g, '').replace('-','');
